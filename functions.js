...
function loadCheckpoint(jsonString){
	...
	try{
		var checkpoints = JSON.parse(document.getElementById('json').value);
	...
	//
			td = tr.insertCell(-1);
			td.align = 'right';
			var seconds = checkpoint.s;
			if (seconds == undefined){
				seconds = 0;
			}
			var minutes = checkpoint.m;
			...
			if (showMilliseconds){
				var milliseconds = checkpoint.ms;
